...
\naslov{Newtonova metoda za dvonivojsko optimizacijo trajektorij diferencialno ploščatih sistemov z analitičnimi odvodi}
...
Matriko $\mathbf{Q}$ izračunamo tako, da ločeno prevedemo posamezen integral $\frac{1}{2}\int_0^{t_i} (p_i^{(r)}(\tau))^2 d\tau$ na matrično enačbo $\mathbf{a}_i^\top \mathbf{Q}_i \mathbf{a}_i$, kjer je $\mathbf{a}_i$ vektor koeficientov $i$-tega polinoma. Celotno vsoto $\frac{1}{2}\sum_{i=1}^n \int_0^{t_i} (p_i^{(r)}(\tau))^2 d\tau$ pa nato predstavimo kot bločno matriko
\begin{equation}
...
Za preoblikovaje integrala $J := \frac{1}{2}\int_0^{t_i} (p_i^{(r)}(\tau))^2 d\tau$ v matrično obliko uporabimo naslednja rezultata:
\begin{enumerate}
...
\noindent še bolj kompaktno pa jih lahko zapišem kot:
...
\begin{izrek}[Izrek o implicitni funkciji \cite{krantz2002implicit}]
    ...
    Fiksirajmo točko $(\mathbf{a}, \mathbf{b}) = (a_1, a_2, \ldots, a_m, b_1, b_2, \ldots, a_n)$, kjer velja $f(\mathbf{a},\mathbf{b}) = \mathbf{0}$ in je $\mathbf{0} \in \mathbb{R}^m$ ničelni vektor. Če je Jacobijeva matrika
    \begin{equation}
        J_\mathbf{y}f(\mathbf{a}, \mathbf{b}) = \left[ \frac{\partial f_i}{\partial y_j} (\mathbf{a},\mathbf{b})\right]
    \end{equation}

    \noindent obrnljiva, potem obstaja odprta množica $U \in \mathbb{R}^n$, ki vsebuje $\mathbf{a}$, tako da obstaja enolična funkcija $g:U \to \mathbb{R}^m$, kjer $g(\mathbf{a}) = \mathbf{b}$ in $f(\mathbf{x}, g(\mathbf{x})) = \mathbf{0}$ za vse $\mathbf{x} \in U$. Nadalje, $g$ je zvezno odvedljiva in Jacobijeva matrika parcialnih odvodov $g$ v $U$ je definirana kot:
    \begin{equation}
...
V tem podpoglavju predstavimo rezultate ekseperimentov, s katerimi primerjamo konvergenco sledečih metod:
\begin{enumerate}
