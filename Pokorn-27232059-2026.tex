...
\naslov{Newtonova metoda z analitičnimi odvodi za dvonivojsko optimizacijo trajektorij diferencialno ploščatih sistemov}

% - naslov dela v angleščini
\title{Newton's method with analytical derivatives for bilevel trajectory optimization of differentially flat systems}
...
\abstract{Bilevel optimization is an established approach to continuous trajectory planning for differentially flat systems such as quadrotors, where the lower level is defined as a quadratic program (QP) and the upper level is solved by computing the gradient of the optimal solution of the lower level. The gradient can be computed analytically via the sensitivity analysis of the QP's KKT conditions and the use of the implicit function theorem. In this work we take a step further and analytically compute second-order derivatives of the QP optimal solution, enabling the use of Newton's method and consequently convergence to the optimum in significantly fewer iterations than with existing approaches. Utilising the structures of the cost and constraint matrices we achieve additional speed-ups.}
...
\klasifikacija{90C53, 90C30, 90C20, 49M37, 49M15, 90C31}
...
\DeclareAcronym{QP}{
  short = QP,
  long = kvadratični program,
  long-plural-form = kvadratični programi,
...
\DeclareAcronym{LQR}{
  short = LQR,
  long = linearni kvadratični regulator,
...
\DeclareAcronym{mpec}{
  short = MPEC,
  long = matematično programiranje z ravnovesnimi pogoji,
...
\newcommand{\muimplicit}{\bar{\boldsymbol{\mu}}}
...
% 2. opis problema
V tem magistrskem delu rešujemo problem optimizacije trajektorije brezpilotnika, kjer imamo podan seznam točk v prostoru $P_0, P_1, \ldots, P_n$, skozi katere mora brezpilotnik leteti, ter čas letenja $T$. Pri tem želimo minimizirati povprečen trzaj (3. odvod) ali kak drug odvod trajektorije. Na sliki~\ref{fig:primer_trajektorije} je predstavljen primer optimalne trajektorije z 11 vmesnimi točkami. Medtem ko se da problem, kjer so časi po posameznih segmentih $\tvec = \left[ t_1, t_2, \ldots, t_n \right]$ fiksni, reformulirati kot \ac{QP}, je naš problem, kjer je $\tvec$ vektor prostih spremenljivk, nekonveksen. Tega lahko rešimo tako, da najdemo optimalni $\tvec$ z iterativnim reševanjem \ac{QP}, parametriziranega s fiksnimi časi, kjer se z vsako iteracijo približamo lokalnemu minimumu. Ta pristop je prvič predstavljen v prelomnem članku \cite{mellinger2011minimum}, vendar tu problem s spremenljivimi časi segmentov rešijo z numeričnim izračunom gradienta, kar je računsko zahtevno, saj zahteva več evalvacij cenovne funkcije. V \cite{sun2021fast} z metodo računanja analitičnega gradienta z občutljivostno analizo \ac{KKT} oz. dvonivojsko optimizacijo dosežejo pohitritve. V tem delu na podoben način izračunamo tudi analitične parcialne odvode drugega reda, kar nam omogoči uporabo Newtonove metode, ki konvergira k optimalni rešitvi v manj korakih kot v \cite{sun2021fast}.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.99\textwidth]{images/trajectory_3d_velocity.pdf}
  \caption[Primer trajektorije.]{Različne perspektive primera optimalne trajektorije glede na trzaj (3. odvod) in s končnim časom 6 s, kjer so s črnimi krogi podane vmesne točke, ob njih pa so pripisani pripadajoči vmesni časi.}
...
\begin{itemize}
    \item razvijemo metodo računanja drugih parcialnih odvodov optimalne rešitve \ac{QP} na spodnjem nivoju dvonivojskega optimizacijskega problema, kar nam omogoči uporabo Newtonove metode za optimizacijo zgornjega nivoja;
    ...
    \item nadalje ugotovimo, da je Hessejeva matrika skoraj pasovna in da pri izračunu zgolj nekaj pasov Hessejeve matrike dosežemo pohitritev metode, pri čemer je vpliv izgube izvenpasovnih elementov Hessejeve matrike na konvergenco zanemarljiv.
\end{itemize}

\paragraph{Dostopnost implementacije.}
Implementacija metode, opisane v tem delu, je javno dostopna na GitHub repozitoriju \cite{pokorn2026trajectory}.

% 5. opis preostanka magistrske

Preostanek tega magistrskega dela je strukturiran na sledeč način. Najprej v poglavju~\ref{sec:pregled_podrocja} opišemo sorodna dela ter dela in pojme, ki služijo kot temelj naši metodi. Nato v poglavju~\ref{sec:opis_metode} predstavimo optimizacijski problem in podrobno opišemo našo metodo reševanja in jo teoretično podkrepimo. V poglavju~\ref{sec:eksperimenti} analiziramo implementacijo naše metode v smislu konvergence in hitrosti v odvisnosti od vhodnih parametrov. V poglavju~\ref{sec:zakljucek} povzamemo vsebino in predlagamo izboljšave in nadaljnje delo. V dodatku~\ref{app:example} predstavimo del naše metode na konkretnem primeru, v dodatku~\ref{app:implementation} pa podrobno predstavimo učinkovit izračun parcialnih odvodov cenovne funkcije.
...
V tem poglavju opišemo sorodna dela in predstavimo pojme in dognanja, na katerih je naše delo osnovano. Najprej podamo opis diferencialno ploščatih sistemov in njihovo povezavo z optimizacijo trajektorij. Nato predstavimo trenutno stanje področja optimizacije trajektorij za kvadrotorje in raziskave, na katerih naše delo bodisi temelji bodisi predstavljajo alternativne pristope k reševanju podobnih problemov. Potem opišemo področje dvonivojske optimizacije, na koncu pa še Newtonovo metodo za iskanje ekstremov.

\subsection{Diferencialno ploščati sistemi}
V tem delu se opiramo na pojem diferencialne ploščatosti. Denimo, da imamo podan dinamični sistem, ki ga lahko opišemo z diferencialnimi enačbami:
\begin{equation}
    \dot{x} = f(x, u), \quad x \in \mathbb{R}^n, u \in \mathbb{R}^m,
\end{equation}
kjer je $f: \mathbb{R}^{n+m} \to \mathbb{R}^n$ funkcija, ki določa dinamiko sistema. Rečemo, da je sistem diferencialno ploščat, če lahko najdemo tak nabor izhodnih spremenljivk $y \in \mathbb{R}^m$ (ploščati izhodi), da lahko stanje $x$ in krmilni vhod $u$ izrazimo kot funkciji ploščatih izhodov in njihovih odvodov \cite{murray1995differential}:
\begin{align}
    x &= \phi(y, \dot{y}, \ddot{y}, \dots, y^{(p)}) \\
    u &= \psi(y, \dot{y}, \ddot{y}, \dots, y^{(q)}).
\end{align}

Diferencialno ploščati sistemi upravljanja so koristni v primerih, kot je naš, kjer želimo eksplicitno določiti trajektorije. Tako lahko iz dovolj gladke trajektorije (izhoda) preprosto izračunamo vhod in stanje sistema. Izkaže se, da so tudi kvadrotorji diferencialno ploščati sistemi; postopek prevedbe iz izhodov na vhode sistema kvadrotorja je opisan v \cite{mellinger2011minimum}. V tem delu se osredotočimo na generiranje optimalnih trajektorij, diferencialna ploščatost pa je pomembna, ker želimo, da sistem trajektoriji dejansko sledi.
...
Znan primer dvonivojske optimizacije je Stackelbergova igra \cite{von2010market}, ki je igra med dvema igralcema (voditelj in sledilec), kjer voditelj optimizira svojo strategijo glede na sledilčevo. 

Dvonivojska optimizacija v načrtovanju trajektorij brezpilotnikov in v robotiki na splošno se navadno uporablja, ko je optimizacijski problem odvisen od časovne razporeditve in nekaterih drugih parametrov, ki pa so tudi odvisni od časovne razporeditve. V našem primeru so to koeficienti polinomov. Pogosto se, kot v našem primeru, izkaže, da se da spodnji nivo izraziti kot \ac{QP}, ki se ga da rešiti s standardnimi knjižnicami. Tudi problem v \cite{mellinger2011minimum} je implicitno definiran kot dvonivojski optimizacijski problem. V \cite{sun2021fast}, kjer rešujejo isti problem, poleg izračuna analitičnih gradientov svojo rešitev podkrepijo teoretično. Izven domene načrtovanja trajektorij v \cite{amos2017optnet} dvonivojsko optimizacijo na \ac{QP} uporabijo za integracijo \ac{QP} kot plast v globokih nevronskih mrežah, kjer so parametri \ac{QP} odvisni od drugih plasti v nevronski mreži. V \cite{chapelle2002choosing} na zelo podoben način kot v tem delu (kjer je spodnji nivo \ac{QP}) optimizirajo hiperparametre metode podpornih vektorjev (angl. support vector machines). V \cite{dyro2022second} razvijejo teorijo računanja Hessejeve matrike zgornjega problema splošnih dvonivojskih neomejenih optimizacijskih problemov z uporabo izreka o implicitni funkciji (\acs{IFT}); z eksperimenti tudi pokažejo hitrejšo konvergenco kot obstoječe metode.

\subsection{Newtonova metoda}
Newtonova metoda je iterativna metoda za iskanje ekstremov dvakrat zvezno odvedljivih funkcij \cite{boyd2004convex}. Korak v iteraciji Newtonove metode za funkcije več spremenljivk se izračuna z enačbo

\begin{equation}
    x_{k+1} = x_k - [\nabla^2 f(x_k)]^{-1} \nabla f(x_k), 
\end{equation}
kjer je $f : \mathbb{R}^n \to \mathbb{R}$ dvakrat zvezno odvedljiva funkcija, $x_k \in \mathbb{R}^n$ trenutna točka, $x_{k+1} \in \mathbb{R}^n$ pa naslednja točka iteracije. $\nabla^2f(x_k)$ predstavlja Hessejevo matriko funkcije $f$ v točki $x_k$, $\nabla f(x_k)$ pa gradient funkcije $f$ v točki $x_k$. Kot že omenjeno, v \cite{dyro2022second} razvijejo teorijo računanja gradienta in Hessejeve matrike za neomejene dvonivojske optimizacijske probleme in pokažejo hitro konvergenco. Tudi v tem delu eksperimenti pokažejo zelo hitro konvergenco k optimalni rešitvi.

\section{Opis metode}
\label{sec:opis_metode}

V tem poglavju opišemo našo metodo optimizacije trajektorij. Najprej definiramo osnovni problem v obliki minimizacije funkcionala z omejitvami s fiksnimi potovalnimi časi po segmentih. Nato pokažemo, kako se problem minimizacije funkcionala prevede na QP, kjer optimizacijske spremenljivke postanejo koeficienti polinomov. Dodamo optimizacijo po časih segmentov in predstavimo glavni optimizacijski problem, ki ga v tem delu rešujemo. Nato opišemo, kako lahko s pomočjo KKT pogojev QP izluščimo gradient in Hessejevo matriko cenovne funkcije v zgornjem nivoju dvonivojskega problema. Zatem predstavimo minimizacijski algoritem, ki sloni na Newtonovi metodi. Na koncu pokažemo, kako lahko z uporabo bločne strukture matrik in strukture Hessejeve matrike dosežemo velike pohitritve. 
...
\end{aligned}
\end{equation}
Pravzaprav tak problem rešujemo trikrat, posebej za vsako od osi $x$, $y$ ter $z$, kjer $t_i$ ostane isti, spremenijo pa se zgolj koordinate vmesnih točk $P_i$ za posamezno os. Končni rezultat je tridimenzionalna krivulja. Z nekaj svobode pa lahko izberemo parametre:
\begin{itemize}
...
V praksi se pogosto minimizira pok (\textit{angl. snap}) oz. $r=4$, zahtevana pa je vsaj zveznost trzaja (\textit{angl. jerk}) oz. $K = 3$.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
kjer je $\mathbf{a} = [\avec_1, \avec_2, \ldots, \avec_n] \in \mathbb{R}^{n (d+1)}$ optimizacijski vektor in predstavlja koeficiente polinomskih zlepkov ($\avec_i$ je vektor koeficientov, ki pripadajo $i$-temu segmentu), $\mathbf{Q} \in \mathbb{R}^{n(d+1) \times n(d+1)}$ je simetrična in pozitivno definitna matrika, $\mathbf{A} \in \mathbb{R}^{M \times n(d+1)}$ in $\mathbf{b} \in \mathbb{R}^M$ pa predstavljata linearne omejitve. Najprej bomo pokazali, kako izračunamo matriko $\mathbf{Q}$, nato pa še, kako izračunamo matriko $\mathbf{A}$ in vektor $\mathbf{b}$. Bistvenega pomena pri tej izpeljavi je, da so elementi matrik $\mathbf{Q}$ in $\mathbf{A}$ zvezno odvisni od vmesnih časov $t_i$ \cite{carlone2020vnav}; to nam bo koristilo pri izračunu gradienta in Hessejeve matrike cenovne funkcije v zgornjem nivoju dvonivojskega problema.

\subsubsection{Izračun matrike $\mathbf{Q}$}
Matriko $\mathbf{Q}$ izračunamo tako, da prevedemo posamezen integral $\frac{1}{2}\int_0^{t_i} (p_i^{(r)}(\tau))^2 d\tau$ na matrično enačbo $\mathbf{a}_i^\top \mathbf{Q}_i \mathbf{a}_i$, kjer je $\mathbf{a}_i$ vektor koeficientov $i$-tega polinoma. Celotno vsoto $\frac{1}{2}\sum_{i=1}^n \int_0^{t_i} (p_i^{(r)}(\tau))^2 d\tau$ pa nato predstavimo kot bločno matriko
\begin{equation}
...
Za preoblikovanje integrala $J := \frac{1}{2}\int_0^{t_i} (p_i^{(r)}(\tau))^2 d\tau$ v matrično obliko uporabimo naslednja rezultata:
\begin{enumerate}
...
\begin{equation}
    (p^{(r)}(\tau))^2 = \sum_{k=2r}^{2d} \left( \sum_{l=r}^{d} \prod_{m=0}^{r-1} (l-m)(k-l-m) a_l a_{k-l} \right) \tau^{k-2r}
\end{equation}
...
\begin{equation}
    \begin{aligned}
    \frac{\partial J}{\partial a_u} &= \frac{1}{2}\sum_{k=0}^{2d} \sum_{l=0}^{d} \prod_{m=0}^{r-1} (l-m)(k-l-m) \left(\frac{\partial a_l}{\partial a_u} a_{k-l} + \frac{\partial a_{k-l}}{\partial a_u} a_l \right) \frac{t^{k-2r+1}}{k-2r+1}\\
    &= \sum_{k=0}^{2d} \prod_{m=0}^{r-1} (u-m)(k-u-m) a_{k-u} \frac{t^{k-2r+1}}{k-2r+1}
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
    \frac{\partial^2 J}{\partial a_u \partial a_v} &= \sum_{k=0}^{2d} \prod_{m=0}^{r-1} (v-m)(k-v-m) \frac{\partial a_{k-v}}{\partial a_u} \frac{t^{k-2r+1}}{k-2r+1}\\
    &= \prod_{m=0}^{r-1} (v-m)(u-m) \frac{t^{v+u-2r+1}}{v+u-2r+1}
\end{aligned}
\end{equation}

Pri drugem parcialnem odvodu preostale vsote pa dobimo samo en neničelen člen, kjer $v = k-u$ in $0 \leq v \leq d$.
...
Vektor $\bvec$ je sestavljen iz koordinat vmesnih točk $P_i$ in ničel, ki ustrezajo zahtevam zveznosti trajektorije. Omenimo še, da imata bloka $\amat_1(0)$ in $\amat_n(t_n)$ vrstico manj od ostalih blokov, saj ne vsebujta omejitev zveznosti trajektorije. Podrobnejši primer konstrukcije QP s fiksnimi časi segmentov je podan v dodatku~\ref{app:example}. 
...
\begin{figure}[htbp]
     ...
     \end{subfigure}
     \caption{Primerjava trajektorij, kjer fiksno določimo vmesne čase (a), in kjer izvedemo optimizacijo po vmesnih časih (b). Obe trajektoriji imata končni čas 5s, vendar ima desna trajektorija nižjo optimalno vrednost in tudi nižjo maksimalno hitrost.}
     \label{fig:primerjava_fixed_variable}
\end{figure}

V prejšnjem poglavju smo predstavili problem, kjer so časi segmentov podani kot fiksni parametri problema. Pogosto pa nam je pomemben le celoten čas potovanja po trajektoriji. Na primer, slika~\ref{fig:primerjava_fixed_variable} nudi primerjavo med fiksno določenimi vmesnimi časi in optimiziranimi vmesnimi časi za isto zaporedje vmesnih točk in isti skupen čas. Če nam je vseeno, kakšni so časi na vmesnih točkah, je očitno, da optimizacija časov segmentov vodi do bistveno boljše trajektorije v smislu minimizacije kriterijske funkcije. Zato lahko definiramo želen skupen čas trajektorije $T$ kot vhodni parameter in vektor optimizacijskih spremenljivk $\mathbf{t} = \left[ t_1, t_2, \ldots , t_n\right]$ z omejitvama $\sum_{i=1}^n t_i = T$ in $t_i > 0$.  Sedaj lahko formuliramo nov optimizacijski problem:
...
\noindent kjer problemu računanja $\mathbf{a}^*$ rečemo spodnji nivo dvonivojskega optimizacijskega problema, problemu minimizacije $F(\mathbf{t})$ pa zgornji nivo. Če fiksiramo $\mathbf{t}$, je spodnji nivo preprost QP z enakostnimi omejitvami. S takim problemom se srečujejo v \cite{mellinger2011minimum, sun2021fast}, kjer v neki točki $\mathbf{t}'$ izračunajo gradient $\nabla_\mathbf{t}F(\mathbf{t}')$ in izvedejo gradientni spust. Pri tem za izračun gradienta v \cite{mellinger2011minimum} uporabijo metodo končnih diferenc, v \cite{sun2021fast} pa ga z občutljivostno analizo \ac{KKT} pogojev izračunajo analitično.

To je glavni optimizacijski problem, s katerim se v tem magistrskem delu ukvarjamo in preostanek tega poglavja je posvečen opisu naše metode njegovega reševanja. V naslednjem podpoglavju s pomočjo KKT pogojev in izreka o implicitni funkciji pokažemo, da za vsak $\tvec$ obstaja funkcija $\aimplicit \in \mathcal{C}^\infty$, kjer velja $\aimplicit(\tvec) = \mathbf{a}^*$. S tem pokažemo, da velja tudi $F \in \mathcal{C}^\infty$ in omogočimo izračun analitičnih parcialnih odvodov višjega reda za $F(\mathbf{t})$ in s tem gradienta in Hessejeve matrike.

%Dodatno, $\mathbf{Q}, \mathbf{A} \in \mathcal{C}^{\infty}(\mathbb{R}^n)$.

\subsection{Odvajanje s \acs{KKT} pogoji}

V tem podpoglavju bomo predstavili, kako s pomočjo \ac{KKT} pogojev izračunamo gradient in Hessejevo matriko cenovne funkcije zgornjenivojskega problema. Najprej predstavimo izrek o \ac{KKT} pogojih za splošen optimizacijski program in opišemo lastnosti \ac{KKT} pogojev, če imamo konveksen optimizacijski program. Nato v~\eqref{eq:kkt_matrix} definiramo \acs{KKT} matriko za \ac{QP} z enakostnimi omejitvami. V razdelku~\ref{sec:ift} navedemo \acl{IFT} in pokažemo, da obstaja analitična funkcija $\aimplicit$, kjer velja $\aimplicit(\tvec) = \mathbf{a}^*$, pri čemer je $\avec^*$ optimalna rešitev \ac{QP}, parametriziranega s $\tvec$. S tem argumentiramo izračun parcialnih odvodov $\aimplicit$ ter gradienta in Hessejeve matrike $F(\tvec)$, ki jih predstavimo v razdelkih~\ref{sec:grad} in~\ref{sec:hess}.

Spodaj navedemo standarden izrek o \ac{KKT} pogojih \cite{bertsekas2016nonlinear}.
...
\textbf{Slaterjev pogoj \cite{bertsekas2016nonlinear,boyd2004convex}.} 
...
\item $h_j(x^\circ) = 0$ za vse $j=1,\dots,p$ (enakostna dopustnost).
\end{itemize}

Opazimo lahko, da v primeru, kjer neenakostnih omejitev ni, Slaterjev pogoj velja.

\subsubsection{\acs{KKT} pogoji \acs{QP}}

V primeru našega optimizacijskega problema, kjer imamo kvadratično cenovno funkcijo in enakostne omejitve, lahko privzamemo Slaterjev pogoj, saj nimamo neenakostnih omejitev, \ac{KKT} pogoji pa se zreducirajo na:

\begin{equation}
    \begin{aligned}
        \nabla (\frac{1}{2}\mathbf{a^{*\top}}\mathbf{Q}\mathbf{a^*}) + \sum_{i=1}^m \muvec^*_i \nabla(\mathbf{A}_i \mathbf{a^*}) = \mathbf{0},& \\
        \mathbf{A}_i \mathbf{a^*} - \mathbf{b}_i = 0&, \quad i = 1,\ldots, m,
    \end{aligned}
\end{equation}

\noindent še bolj kompaktno pa jih lahko zapišemo kot:

\begin{equation}
    \begin{aligned}
        \mathbf{Q}\mathbf{a^*} + \mathbf{A^\top}\muvec^* = \mathbf{0},& \\
        \mathbf{A} \mathbf{a^*} = \mathbf{b}&.
    \end{aligned}
\end{equation}

Pri optimalnosti torej velja naslednja matrična enačba, kjer so $\muvec$ enakostni Lagrangeevi multiplikatorji, $\mathbf{a^*}$ ter $\muvec^*$ pa so vrednosti $\mathbf{a}$ ter $\muvec$ pri optimalnosti:
\begin{equation}
    \label{eq:kkt_matrix}
    \begin{gathered}
        % Top Equation
...
Spodnji optimizacijski problem v enačbi~\eqref{eq:bilevel} bi torej lahko nadomestili z zgornjo matrično enačbo. V literaturi se področju, ki se ukvarja s takimi problemi, reče tudi \ac{mpec}. \ac{KKT} matrika nam bo koristila pri izračunu gradienta in Hessejeve matrike funkcije $F(\tvec)$; v $F$ namreč nastopa optimalna vrednost $\mathbf{a}^*$, ki je odvisna od $\tvec$, \ac{KKT} matrika pa nam razkrije odvisnost $\mathbf{a}^*$ od $\tvec$. V nadaljevanju s pomočjo \ac{IFT} pokažemo, da lahko $\mathbf{a}^*$ nadomestimo z gladko funkcijo $\aimplicit(\tvec)$, izračunamo njene parcialne odvode in s tem izračunamo gradient in Hessejevo matriko funkcije $F(\tvec)$.

V nadaljevanju bomo izmenično uporabljali tako osnovno kot kompaktno obliko ($\kmat \mathbf{y}^* - \mathbf{c} = \mathbf{0}$) zapisa \ac{KKT} pogojev. Kompaktno obliko bomo uporabili, kjer bo to pripomoglo k boljši preglednosti, osnovno obliko pa tam, kjer bo pomembna struktura posameznih delov matrike $\kmat$, ali pa bo treba ločiti med primarnimi in dualnimi komponentami $\mathbf{y}^*$.

\subsubsection{Izrek o implicitni funkciji}
\label{sec:ift}

Z uporabo izreka o implicitni funkciji lahko pokažemo, da obstaja odprta množica $U$ in gladke funkcije $\yimplicit$ : $U \to \R^{m+n}$, $\aimplicit$ : $U \to \R^{n}$, $\muimplicit$ : $U \to \R^{m}$, da velja $\yimplicit(\tvec) = (\aimplicit(\tvec), \muimplicit(\tvec)) = (\avec^*, \muvec^*)$. Tu sta $\avec^*$ in $\muvec^*$ optimalni vrednosti primarnih in dualnih spremenljivk \ac{QP}, parametriziranega s $\tvec$. To nam da formalno podlago, da cenovno funkcijo $F(\tvec)$ odvajamo glede na $\tvec$, saj jo lahko zapišemo kot:

\begin{equation}
    \label{eq:cost_function_implicit}
...
V tem razdelku najprej podamo izrek o implicitni funkciji \cite{krantz2002implicit} in nato pokažemo, kako z njim argumentiramo obstoj gladkih funkcij $\yimplicit$, $\aimplicit$ in $\muimplicit$.

\begin{izrek}[Izrek o implicitni funkciji \cite{krantz2002implicit}]
    Naj bo $f: \mathbb{R}^{m+n} \to \mathbb{R}^m$ zvezno odvedljiva funkcija in naj ima $\mathbb{R}^{m+n}$ koordinate $(\mathbf{x},\mathbf{y})$. 
    
    Fiksirajmo točko $(\mathbf{a}, \mathbf{b}) = (a_1, a_2, \ldots, a_m, b_1, b_2, \ldots, b_n)$, kjer velja $f(\mathbf{a},\mathbf{b}) = \mathbf{0}$ in je $\mathbf{0} \in \mathbb{R}^m$ ničelni vektor. Če je Jacobijeva matrika
    \begin{equation}
        J_\mathbf{y}f(\mathbf{a}, \mathbf{b}) = \left[ \frac{\partial f_i}{\partial y_j} (\mathbf{a},\mathbf{b})\right]
    \end{equation}

    \noindent obrnljiva, potem obstaja odprta množica $U \subset \mathbb{R}^n$, ki vsebuje $\mathbf{a}$, tako da obstaja enolična funkcija $g:U \to \mathbb{R}^m$, kjer $g(\mathbf{a}) = \mathbf{b}$ in $f(\mathbf{x}, g(\mathbf{x})) = \mathbf{0}$ za vse $\mathbf{x} \in U$. Nadalje, $g$ je zvezno odvedljiva in Jacobijeva matrika parcialnih odvodov $g$ v $U$ je definirana kot:
    \begin{equation}
        \left[\frac{\partial g_i}{\partial x_j}(\mathbf{x})\right]_{m \times n}
= -\left[ J_\mathbf{y}f(\mathbf{x}, g(\mathbf{x})) \right]^{-1}_{m \times m}
\left[ J_\mathbf{x}f(\mathbf{x}, g(\mathbf{x})) \right]_{m \times n},
    \end{equation} kjer je 

\begin{equation}
        J_\mathbf{x}f(\mathbf{a}, \mathbf{b}) = \left[ \frac{\partial f_i}{\partial x_j} (\mathbf{a},\mathbf{b})\right].
    \end{equation}
\end{izrek}

\begin{izrek} [Gladkost rešitve \acs{KKT}]
Naj bo $\tvec > \mathbf{0}$ nek vektor in naj velja enačba $\kmat(\tvec)\mathbf{y}^* - \mathbf{c} = \mathbf{0}$, kjer je $\mathbf{y}^* = (\avec^*, \muvec^*)$ rešitev \ac{KKT} \ac{QP} z enakostnimi omejitvami, parametriziranega s $\tvec$. Potem obstaja odprta množica $U \subset \mathbb{R}^n$ in enolične gladke funkcije $\yimplicit : U \to \mathbb{R}^{m+n}$, $\aimplicit : U \to \mathbb{R}^{n}$, $\muimplicit : U \to \mathbb{R}^{m}$, kjer velja $\yimplicit(\tvec) = (\aimplicit(\tvec), \muimplicit(\tvec)) = (\avec^*, \muvec^*)$.
\end{izrek}

\begin{dokaz}
Pokazati moramo, da je Jacobijeva matrika parcialnih odvodov enačbe \newline $\kmat \mathbf{y}^* - \mathbf{c}$ glede na $\mathbf{y}^*$ obrnljiva. Ni težko videti, da je Jacobijeva matrika parcialnih odvodov kar \acs{KKT} matrika $\mathbf{K}$. Spomnimo, matrika $\kmat$ ima bločno strukturo in je definirana v enačbi~\eqref{eq:kkt_matrix}. Ker je $\mathbf{Q}$ pozitivno definitna na $\ker(\amat)$ in ker ima $\mathbf{A}$ poln rang, je matrika $\mathbf{K}$ obrnljiva (dokaz za to je podan v \cite{EPFL_Part5}). S tem je pogoj \ac{IFT} izpolnjen, kar zagotavlja obstoj in enoličnost gladkih funkcij $\yimplicit$, $\aimplicit$ in $\muimplicit$.
\end{dokaz}

\subsubsection{Računanje gradienta}
\label{sec:grad}

V tem razdelku predstavimo, kako izračunamo gradient cenovne funkcije $F(\tvec)$, predstavljene v enačbi~\eqref{eq:cost_function_implicit}. Začnemo z zapisom parcialnega odvoda $F(\tvec)$ glede na komponento $t_i$:
\begin{equation}
    \label{eq:grad}
    \partial_{t_i} F = \aimplicit^\top \qmat (\partial_{t_i} \aimplicit) + \frac{1}{2} \aimplicit^\top (\partial_{t_i} \qmat) \aimplicit
\end{equation}

Za izračun $\partial_{t_i}F$ torej potrebujemo 4 komponente: $\aimplicit$, $\qmat$, $\partial_{t_i}\aimplicit$ in $\partial_{t_i}\qmat$. $\qmat$ poznamo, $\aimplicit$ je $\avec^*$, $\partial_{t_i}\qmat$ lahko izračunamo z odvajanjem $\qmat$ po komponentah, saj so vsi elementi $\qmat$ zvezno odvisni od $\tvec$. Najzahtevnejši pa je izračun $\partial_{t_i}\aimplicit$, saj ne poznamo eksplicitne formule za $\aimplicit$. Za izračun $\partial_{t_i}\aimplicit$ uporabimo rezultat iz prejšnjega razdelka; iz enačbe~\eqref{eq:equilibrium_implicit} lahko izrazimo parcialni odvod $\partial_{t_i} \yimplicit(\tvec)$ kot:
\begin{equation}
    \label{eq:dkkt}
    \begin{aligned}
...
Iz $\partial_{t_i} \yimplicit(\tvec)$ lahko sedaj vzamemo komponento $\partial_{t_i} \aimplicit(\tvec)$ in jo vstavimo v izraz za $\partial_{t_i} F(\tvec)$. Ko ta postopek ponovimo za vse komponente $\tvec$, dobimo gradient $F(\tvec)$:
\begin{equation}
    \nabla F(\tvec) = [\partial_{t_1} F(\tvec), \partial_{t_2} F(\tvec), \dots, \partial_{t_n} F(\tvec)]
\end{equation}

\subsubsection{Računanje Hessejeve matrike}
\label{sec:hess}

V tem razdelku predstavimo, kako izračunamo Hessejevo matriko cenovne funkcije $F(\tvec)$. Začnemo z zapisom parcialnega odvoda $\partial_{t_j}\partial_{t_i} F(\tvec)$ glede na komponenti $t_i$ in $t_j$, kjer prvi parcialni odvod, izračunan v enačbi~\eqref{eq:grad}, še enkrat odvajamo po komponenti $t_j$. Uporabimo produktno pravilo za odvode:
...
Tudi tu moramo določiti vrednost več komponent; razen $\partial_{t_it_j} \qmat$ in $\partial_{t_it_j} \aimplicit$ smo ji izračunali že med postopkom izračuna gradienta. $\partial_{t_it_j} \qmat$ lahko izračunamo tako, da še enkrat odvajamo $\partial_{t_i} \qmat$ po $t_j$, za $\partial_{t_it_j} \aimplicit$ pa se spet poslužimo \ac{IFT}. Rešimo spodnji sistem enačb, ki ga dobimo z odvajanjem obeh strani sistema v enačbi~\eqref{eq:dkkt} po $t_j$:
\begin{equation}
    \label{eq:ddkkt}
...
\begin{equation}
    \label{eq:projection}
    \nabla F_{proj}(\tvec) = \nabla F(\tvec) - \frac{\nabla F(\tvec)^\top \mathbf{1}}{\mathbf{1}^\top \mathbf{1}} \mathbf{1} = \nabla F(\tvec) - \frac{1}{n} \sum_{i=1}^n \partial_{t_i} F(\tvec) \mathbf{1}
\end{equation}
...
\end{bmatrix}
\end{equation}
To je standarden pristop pri Newtonovi metodi z enakostnimi omejitvami \cite{nocedal2006numerical}, kjer $\lambda$ predstavlja Lagrangeeve multiplikatorje za omejitev $\mathbf{1}^\top \mathbf{t} = T$. Rešitev tega sistema nam da smer $\Delta \tvec$, v katero bomo naredili korak:
\begin{equation}
    \tvec_{k+1} = \tvec_k + \alpha \Delta \tvec_k.
\end{equation}

V nadaljevanju bomo veliko pozornost namenili reševanju sistemov enačb~\eqref{eq:dkkt} in~\eqref{eq:ddkkt}, zato bi bilo morda na prvi pogled smiselno optimizirati tudi reševanje tega sistema. Vendar pa so sistemi v enačbah~\eqref{eq:dkkt} in~\eqref{eq:ddkkt} za problem iste velikosti mnogo večji kot sistem, opisan v enačbi~\eqref{eq:upper_system}, prav tako pa imajo več desnih strani. Zato so tudi ozko grlo te metode v smislu računske zahtevnosti, čas reševanja tega sistema pa je v primerjavi zanemarljiv.

\subsubsection{Optimizacijski algoritem}

V tem razdelku predstavimo celoten optimizacijski algoritem, ki ga uporabljamo za minimizacijo $F(\tvec)$ in je predstavljen v algoritmu~\ref{alg:newton}. V njem iterativno izračunavamo gradient in Hessejevo matriko cenovne funkcije $F(\tvec)$ z algoritmom~\ref{alg:gradhess}. Uporabljamo hibridno linijsko iskanje z vračanjem, kjer se lahko izmenjujeta Newtonov in gradientni korak glede na to, kateri korak ponuja večje zmanjšanje funkcije $F(\tvec)$; predvsem v začetnih iteracijah je pogosto bolj učinkovit gradientni korak, v končnih iteracijah pa Newtonov korak. Na vsakem koraku optimizacije izračunamo spodnjenivojski problem; ločeno izračunamo \ac{QP} za vsako izmed osi $x$, $y$ in $z$. Nato ekstrahiramo primarne in dualne optimalne vrednosti $\avec^*$ in $\muvec^*$ ter izračunamo gradient in Hessejevo matriko s postopkom opisanim v prejšnjih poglavjih. Pri linijskem iskanju z vračanjem za kriterij uporabljamo Armijo-Goldsteinov pogoj \cite{nocedal2006numerical}.

\begin{algorithm}[H]
\caption{Izračun gradienta in Hessejeve matrike (gradHess)}
\label{alg:gradhess}
\begin{algorithmic}[1]
\REQUIRE Matrika $\qmat$, matrika $\amat$, optimalna rešitev $\avec^*$, Lagrangeevi multiplikatorji $\muvec^*$, trenutni vektor časov $\tvec$
\ENSURE Gradient $\nabla F(\tvec)$ in Hessejeva matrika $\nabla^2 F(\tvec)$
...
\FOR {$i = 1$ to $n$}
    \FOR {$j = i$ to $n$}
        ...
        \STATE Izračunaj komponento Hessejeve matrike $\partial_{t_it_j} F$ z enačbo~\eqref{eq:hess}
    \ENDFOR
\ENDFOR
\RETURN $\nabla F(\tvec)$, $\nabla^2 F(\tvec)$
\end{algorithmic}
...
Algoritem~\ref{alg:gradhess} opisuje računanje gradienta in Hessejeve matrike po komponentah, kar, kot je razvidno v psevdokodi, zahteva gnezdenje zank in je z vidika implementacije precej neučinkovito. V naši implementaciji smo ta algoritem optimizirali tako, da smo s pomočjo vektorizacije in vektorskih operacij, ki jih ponuja knjižnica NumPy \cite{harris2020array}, izračunali vse parcialne odvode $\partial_{t_i} \aimplicit$ in $\partial_{t_it_j} \aimplicit$ v enem klicu funkcije, namesto da bi zanko po komponentah izvajali ločeno. Natančnejši opis optimizacije algoritma za računanje gradienta je podan v dodatku~\ref{app:implementation}.
...
\FOR {$k = 0$ to $K$}
    ...
    \STATE Izračunaj $\nabla F(\tvec_k) = \nabla F_x(\tvec_k) + \nabla F_y(\tvec_k) + \nabla F_z(\tvec_k)$
    \STATE Izračunaj $\nabla^2 F(\tvec_k) = \nabla^2 F_x(\tvec_k) + \nabla^2 F_y(\tvec_k) + \nabla^2 F_z(\tvec_k)$
    \STATE Izračunaj normaliziran projicirani gradientni korak $\mathbf{\tilde{p}}(\tvec_k)$.
...
V prejšnjih podpoglavjih smo predstavili metodo za izračun analitičnega gradienta in Hessejeve matrike cenovne funkcije $F(\tvec)$, kar nam omogoča uporabo Newtonove metode z linijskim iskanjem z vračanjem (algoritem~\ref{alg:newton}) za minimizacijo $F(\tvec)$. V tem podpoglavju se bomo posvetili podrobnostim implementacije, ki so ključne za doseganje boljše časovne učinkovitosti naše metode. Najprej predstavimo, kako faktoriziramo matriko $\kmat$, kjer izkoristimo njeno redko strukturo. Nato opišemo metodo izračuna desne strani sistemov linearnih enačb pri računanju prvih in drugih parcialnih odvodov $F$. Na koncu predstavimo pasovno strukturo Hessejeve matrike, ki nam omogoča dodatno pohitritev optimizacijskega algoritma.
...
\begin{equation}
    ...
    \end{bmatrix}
\end{equation}
V poglavju~\ref{sec:prevedba} smo pokazali, da je matrika $\qmat$ bločno diagonalna matrika, matrika $\amat$ pa je bločno bidiagonalna matrika (ima neničelne bloke na glavni diagonali in prvi poddiagonali). Ker ima $\qmat$ $n$, $\amat$ pa $2n$ neničelnih blokov, je torej matrika $\kmat$ redka matrika z $O(n)$ neničelnimi elementi (natančneje, ima $5n$ neničelnih blokov), kjer je $n$ število segmentov trajektorije. Zaradi tega je smiselno uporabiti metodo faktorizacije, ki je prilagojena za redke matrike, kot je SuperLU \cite{superlu_ug99}, namesto da bi izračunali inverz matrike $\kmat$, kar bi bilo računsko bolj zahtevno.

\subsubsection{Računanje desne strani sistemov linearnih enačb}
\label{sec:rhs}

V poglavjih~\ref{sec:grad} in~\ref{sec:hess} smo pokazali, da za izračun analitičnega gradienta in Hessejeve matrike cenovne funkcije $F(\tvec)$ potrebujemo rešiti linearna sistema enačb z matriko $\kmat$ in desno stranjo, ki vključuje parcialne odvodne matrike $\partial_{t_i}\kmat$ in $\partial_{t_it_j}\kmat$. V tem podpoglavju bomo predstavili, kako učinkovito izračunati te desne strani, pri čemer bomo izkoristili strukturo matrike $\kmat$ in njene parcialne odvodne matrike.

Naš glavni cilj je učinkovito izračunati desne strani sistemov linearnih enačb, ki jih potrebujemo za izračun gradienta in Hessejeve matrike. Za izračun gradienta potrebujemo izraz:
\begin{equation}
...
\end{equation}
Za izračun Hessejeve matrike pa potrebujemo izraz:
\begin{equation}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
lahko vidimo, da moramo izračunati parcialne odvode matrik $\partial_{t_i}\qmat$, $\partial_{t_i} \amat$, $\partial_{t_it_j}\qmat$ in $\partial_{t_it_j} \amat$. Vsako od njih obravnavamo posamezno.
...
Pokazali smo, da so parcialni odvodi matrike $\kmat$ redke matrike. Njihova struktura je bločna, pri čemer so največ trije bloki neničelni, njihova velikost pa je odvisna od stopnje polinoma $d$ in želene stopnje gladkosti trajektorije $K$, ne pa tudi od števila segmentov $n$. Lahko torej trdimo, da je $nnz(\partial_{t_i}\kmat)$ in $nnz(\partial_{t_it_j}\kmat)$ $O(1)$ v odvisnosti od števila segmentov $n$, kjer $nnz$ predstavlja število neničelnih (\textit{angl. non-zero}) elementov matrike.

Sedaj lahko izračunamo desno stran sistemov linearnih enačb tako, da izračunamo le neničelne elemente vektorjev $\mathbf{B}_i$ in $\mathbf{B}_{ij}$. Upoštevajoč specifično bločno strukturo $\partial_{t_i}\kmat$, se lahko povsem izognemo računanju z ničelnimi bloki in $\mathbf{B}_i$ izračunamo kot:
...
Jasno postane, da je tudi $\mathbf{B}_i$ redka matrika z $O(1)$ neničelnimi elementi v odvisnosti od števila segmentov $n$. Podobno lahko izračunamo desno stran sistemov linearnih enačb za Hessejevo matriko, kjer izračun postane kompleksnejši:
...
Tu moramo upoštevati, da so parcialni odvodi $\partial_{t_it_j}\qmat$ in $\partial_{t_it_j}\amat$ ničelni, če je $i \neq j$. Zato obravnavamo dva primera: $i=j$ in $i \neq j$.

V primeru $i=j$ so določeni členi vsote enaki in lahko izračunamo $B_{ii}$ kot:

\begin{equation}
    \begin{aligned}
        \mathbf{B}_{ii} =& -\begin{bmatrix}
...
V primeru $i \neq j$ pa člena s $\partial_{t_it_j}\qmat$ in $\partial_{t_it_j}\amat$ izgineta, vendar pa različni ustvarijo štiri neničelne dele vektorja $B_{ij}$, ki jih lahko izračunamo kot:

\begin{equation}
    \begin{aligned}
        \mathbf{B}_{ij} = \mathbf{B}_{ji} =& - \begin{bmatrix}
...
Vidimo lahko, da sama konstrukcija vektorjev $\mathbf{B}_i$ in $\mathbf{B}_{ij}$ zahteva $O(1)$ operacij glede na $n$, saj imajo bloki konstantno velikost ($\qmat_i \in \mathbb{R}^{(d+1)\times(d+1)}$ in $\amat_i \in \mathbb{R}^{(K+2) \times (d+1)}$). V dodatku \ref{app:implementation} pokažemo, kako v primeru gradienta lahko vektoriziramo opisani proces in namesto računanja v zanki po komponentah izračunamo vse $\mathbf{B}_i$ naenkrat s pomočjo tenzorjev. Enako lahko storimo tudi za Hessejevo matriko, vendar v tem primeru bralca napotimo na implementacijo.

Ne glede na pohitritve izračuna desnih strani oziroma komponent $B_i$ in $B_{ij}$, pa je reševanje linearnih sistemov enačb s \ac{KKT} matriko $\kmat$ še vedno časovno zahtevno, saj je potrebno rešiti $n$ sistemov linearnih enačb za izračun gradienta in $\frac{n(n+1)}{2}$ sistemov linearnih enačb za izračun Hessejeve matrike. V naslednjem poglavju naslovimo kvadratično število desnih strani sistemov linearnih enačb, ki jih moramo rešiti za izračun Hessejeve matrike s tem, da pokažemo, da je mogoče doseči dober približek Hessejeve matrike z upoštevanjem le njenih pasov okoli diagonale.

\subsubsection{Pasovna struktura Hessejeve matrike}
\label{sec:hess_band}

Newtonova metoda v splošnem zahteva izračun in faktorizacijo Hessejeve matrike, kar je lahko za probleme z veliko spremenljivkami računsko zahtevno. Če računamo vsak parcialni odvod drugega reda cenovne funkcije $F(\tvec)$, potem je število parcialnih odvodov drugega reda $\frac{n(n+1)}{2}$, kjer je $n$ število segmentov trajektorije. V našem primeru pa se izkaže, da samo z izračunom diagonale in nekaj pasov okoli nje dosežemo zelo dober približek Hessejeve matrike, s tem pa tudi znižamo časovno zahtevnost na $O(n)$. Na sliki \ref{fig:hess_band} je prikazana Hessejeva matrika na primeru z $n=39$ segmenti trajektorije. Vidimo lahko, da so neničelni elementi matrike skoncentrirani v pasovih okoli diagonale. V podpoglavju~\ref{sec:bands_analysis} bomo pokazali, da je uporaba samo diagonale in 4 oziroma 5 pasov pod in nad diagonalo dovolj za doseganje dobre konvergence optimizacijskega algoritma, hkrati pa nam omogoči pohitritev algoritma.

\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{images/hessian.pdf}
    \caption{Pasovna struktura Hessejeve matrike za $n=39$ segmentov trajektorije.}
    \label{fig:hess_band}
\end{figure}
...
V tem poglavju predstavimo rezultate eksperimentov, ki smo jih izvedli za oceno učinkovitosti naše metode. Najprej predstavimo implementacijske podrobnosti in uporabljene knjižnice. Nato analiziramo konvergenco naše metode, ki jo primerjamo z dvema drugima metodama, in sicer z metodo, ki uporablja samo analitično izračunani gradient, in metodo trust-region, ki uporablja numerično izračunani gradient in Hessejevo matriko. Poleg tega analiziramo hitrost naše metode in njeno odvisnost od števila segmentov trajektorije. Na koncu proučimo pasovno strukturo Hessejeve matrike, kjer nas zanima kvaliteta algoritma glede na število pasov Hessejeve matrike, ki jih upoštevamo pri optimizaciji.

\subsection{Implementacija in uporaba zunanjih knjižnic}
Naša metoda je implementirana v programskem jeziku Python in je dostopna na povezavi \cite{pokorn2026trajectory}. V implementaciji se često zanašamo na operacije z redkimi matrikami za doseganje višje hitrosti in zato uporabljamo modul \texttt{scipy.sparse} knjižnice SciPy \cite{2020SciPy-NMeth}. Konkretno se poslužujemo metode SuperLU za faktorizacijo matrike v enačbi~\eqref{eq:kkt_matrix} \cite{superlu_ug99}. Za reševanje QP uporabljamo knjižnico OSQP \cite{osqp}. Implementacijo naše metode sta omogočili še knjižnici NumPy \cite{harris2020array} in Matplotlib \cite{Hunter:2007}.

\paragraph{Primer uporabe}
Tukaj prikažemo preprost primer uporabe naše metode v programskem jeziku Python. Spodaj je prikazana koda, kjer definiramo problem z $11$ vmesnimi točkami in skupnim časom trajektorije $T=10$. Nato uporabimo metodo, ki implementira algoritem~\ref{alg:newton}. V vrstici \texttt{solver = ...} določimo stopnjo polinoma, kateri odvod želimo optimizirati, stopnjo gladkosti trajektorije (v kodi parameter \texttt{q} predstavlja stopnjo gladkosti $K$) in dimenzionalnost prostora, v katerem se nahajajo točke.
...
V tem podpoglavju predstavimo rezultate eksperimentov, s katerimi primerjamo konvergenco sledečih metod:
\begin{enumerate}
    \item Newtonova metoda, ki uporablja analitično izračunani gradient in Hessejevo matriko, kot je opisano v algoritmu~\ref{alg:newton}.
    ...
    \item Dve metodi trust-constr (TC) iz paketa SciPy \cite{2020SciPy-NMeth}, ki uporabljata numerično oz. analitično izračunani gradient in Hessejevo matriko ter metodo območja zaupanja \cite{conn2000trust}, ki dopušča linearne omejitve.
\end{enumerate}
...
Metodo poženemo tudi na različnem številu točk. Natančneje, izberemo si primere s $5$, $10$, $20$, $30$ in $40$ točkami.
...
\subsection{Pasovna struktura Hessejeve matrike}
\label{sec:bands_analysis}

V podpoglavju~\ref{sec:hess_band} smo namignili, da ima Hessejeva matrika strukturo, kjer so neničelni elementi skoncentrirani na diagonali in nekaj pasovih okoli nje, kar pomeni, da nam elementov matrike izven teh pasov ni treba izračunati, a še vedno dobimo dober približek Hessejeve matrike. Na sliki \ref{fig:hess_bands_comparison} je prikazana primerjava polne Hessejeve matrike in aproksimacij z različnim številom pasov okoli diagonale.
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{Primerjava polne Hessejeve matrike in aproksimacij z različnim številom pasov okoli diagonale.}
    \label{fig:hess_bands_comparison}
\end{figure}

V tem podpoglavju podrobneje analiziramo vpliv števila izračunanih pasov Hessejeve matrike na konvergenco optimizacijskega algoritma. Pri tem opazujemo tako povprečen čas izračuna kot tudi število iteracij, potrebnih za konvergenco algoritma. Analizo izvedemo na naključnih primerih z 20, 30 in 40 vmesnimi točkami, zanima pa nas, kako se spreminjata konvergenca in čas izračuna glede na število pasov Hessejeve matrike, ki jih izračunamo. Rezultati so prikazani na slikah~\ref{fig:bands_20},~\ref{fig:bands_30} in~\ref{fig:bands_40}. Na levem grafu lahko tako kot v prejšnjem podpoglavju vidimo povprečno izboljšavo cenovne funkcije glede na iteracijo. Na srednjem grafu lahko vidimo celoten povprečen čas v sekundah, ki ga algoritem potrebuje do konvergence, na desnem grafu pa povprečen čas, ki ga potrebuje, da izvede eno iteracijo. Vidimo lahko, da je v vseh testiranih primerih optimalno število pasov okoli 4 ali 5, kar nam omogoča pohitritev algoritma brez večje izgube v hitrosti konvergence. Pri primerih s 30 in 40 segmenti se vidi, da je v primeru 1 pasu čas na iteracijo krajši kot v primeru s 4 ali 5 pasovi, a je število iteracij, potrebnih za konvergenco, večje zaradi slabega približka Hessejeve matrike, kar vodi do večjega skupnega časa izračuna. V primeru z 2 pasovoma pa je v vseh primerih tako povprečen čas do konvergence kot povprečen čas na iteracijo bistveno višji od vseh ostalih.

\begin{figure}
    \centering
    \includegraphics[width=0.99\textwidth]{images/bands_performance_n20.pdf}
    \caption{Vpliv števila izračunanih pasov Hessejeve matrike na konvergenco za 20 vmesnih točk. Vidimo lahko, da je 5 pasov v tem primeru optimalno za najhitrejši izračun.}
    \label{fig:bands_20}
\end{figure}
...
\begin{figure}
    ...
    \caption{Vpliv števila izračunanih pasov Hessejeve matrike na konvergenco za 30 vmesnih točk. Vidimo lahko, da so 4 pasovi v tem primeru optimalno za najhitrejši izračun.}
    \label{fig:bands_30}
\end{figure}
...
\end{aligned}
\end{equation}
Torej, med točko $P_0$ in $P_1$ bomo potovali $1s$, med točko $P_1$ in $P_2$ pa $2s$. Sedaj bomo izračunali matriko $\mathbf{Q}$ po enačbi~\eqref{eq:q}, ki bo imela bločno diagonalno strukturo:
\begin{equation}
\mathbf{Q} = 
...
Matriko $\amat$ izračunamo na podlagi strukture, opisane v enačbi~\eqref{eq:a}. Za naš primer z dvema segmentoma in omejitvami do drugega odvoda dobimo:
\begin{equation}
\amat = 
\begin{bNiceMatrix}[columns-width=1.2em]
...
Pri tem smo z različnimi barvami poudarili bločno strukturo matrik in določili bloke $\textcolor{block1color}{\mathbf{Q}_1(t_1)}$, $\textcolor{block2color}{\mathbf{Q}_2(t_2)}$, $\textcolor{block3color}{\amat_1(0)}$, $\textcolor{block4color}{\left[{\amat_1(t_1)} \text{ } {-\amat_2(0)}\right]}$ in $\textcolor{block6color}{\amat_2(t_2)}$. Tu si lahko tudi ogledamo, kako so omejitve funkcionala iz enačbe~\eqref{eq:functional} zapisane v matrični obliki. Blok $\textcolor{block3color}{\amat_1(0)}$ predstavlja začetne pogoje trajektorije, v zaporedju po vrsticah si sledijo $p_1(0) = 1$, $p_1'(0) = 0$ in $p_1''(0) = 0$. Bloka $\textcolor{block4color}{\left[{\amat_1(t_1)} \text{ } {-\amat_2(0)}\right]}$ predstavljata zveznost trajektorije v točki $P_1$, kjer se segmenta stikata, in sicer $p_1(t_1) = p_2(0)$, $p_1'(t_1) = p_2'(0)$ in $p_1''(t_1) = p_2''(0)$. Zadnja vrstica v teh blokih predstavlja omejitev $p_2(0) = 1$ (torej, prva in zadnja vrstica blokov skupaj določata omejitev $p_1(t_1)=p_2(0)= 1$). Na koncu blok $\textcolor{block6color}{\amat_2(t_2)}$ predstavlja končne pogoje trajektorije, kjer imamo pogoje $p_2(t_2) = 2$, $p_2'(t_2) = 0$ in $p_2''(t_2) = 0$.

Sedaj lahko ločeno rešimo \ac{QP} za $x$ in $y$ dimenzijo:
\begin{equation}
    \begin{aligned}
...
Za izračun gradienta cenovne funkcije $F(\tvec)$ potrebujemo parcialne odvode matrik $\qmat$ in $\amat$ glede na časovne spremenljivke $t_1$ in $t_2$, kot opisano v podpoglavju~\ref{sec:grad}. V tem razdelku predstavimo te parcialne odvode v simbolični in numerični obliki.
...
Tu je zelo jasno vidna ugotovitev iz razdelka~\ref{sec:rhs}, da so matrike $\partial_{t_i} \qmat$ in $\partial_{t_i} \amat$ ničelne z izjemo enega bloka.

Sedaj lahko sestavimo parcialni odvod celotne KKT matrike $\kmat$:
...
s tem pa izračunamo tudi parcialne odvode implicitne funkcije $\yimplicit_x$ (vektor polinomskih koeficientov in Lagrangeevih multiplikatorjev za dimenzijo $x$) glede na $t_1$:
...
V tem dodatku se posvetimo podrobnostim implementacije hitrega izračuna analitičnega gradienta in Hessejeve matrike cenovne funkcije $F(\tvec)$. V prejšnjih poglavjih smo zavoljo jasnosti odvode različnih funkcij in matrik opisovali po komponentah (na primer, $\partial_{t_i} F$, $\partial_{t_i t_j} \aimplicit$, $\partial_{t_it_i} \qmat$, ...), vendar pa je v implementaciji smiselno uporabiti vektorizacijo in vektorske operacije, da se izognemo zankam in nepotrebnemu računanju z ničelnimi bloki. V tem dodatku bomo opisali, kako smo z vektorizacijo izračunali Hessejevo matriko $\nabla^2 F(\tvec)$ in gradient $\nabla F(\tvec)$. Najprej predstavimo, kako smo prestrukturirali vhodne komponente algoritma~\ref{alg:gradhess} v tenzorje brez ničelnih blokov, nato pa s pomočjo tenzorjev in tenzorskih operacij opišemo, kako smo izračunali gradient in Hessejevo matriko. Najprej pa na kratko predstavimo tenzorsko notacijo.
